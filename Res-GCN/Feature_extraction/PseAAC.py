...
fastas = readFasta.readFasta(r"Y_train.txt")
kw={'path': r"PAAC",'train':r"Y_train.txt",'order':'ARNDCQEGHILKMFPSTWYV'}


result=PAAC(fastas, lambdaValue=4, w=0.05, **kw)#4是参数
data=np.matrix(result[1:])
data_PseAAC=pd.DataFrame(data=data[:,1:])
data_PseAAC.to_csv('PseAAC_Y_train.csv')
